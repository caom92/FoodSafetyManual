...
function gmpPackingDocRegistryForm(data){
    var form = {"type":"form","id":"document_registry_form","name":"document_registry_form","method":"post","form":{"sections":[]}};

    var entryNumber = 0;
    for(var log of data){
        log.no = entryNumber++;
        console.log(log);
        form.form.sections.append(gmpPackingDocRegistryItem(log));
    }
...
function gmpPackingDocRegistryItem(item){
    var documentItem = {"type":"section","rows":[]};
    var identityRow = {"type":"row","columns":[]};
    var dataRow = {"type":"row","columns":[]};
    var filesRow = {"type":"row","columns":[]};

    identityRow.columns = [gmpPackingDocRegistryItemDate(item), gmpPackingDocRegistryItemUser(item)];
...
function gmpPackingDocRegistryItemDate(item){
    ...
    var dataFullInput = {"id":"dateWrapper_" + item.id,"classes":"input-field col s4 m4 l4","field":dataInput,"label":dataLabel};
...
function gmpPackingDocRegistryItemUser(item){
    ...
    var userInput = {"type":"input","id":"user_" + item.id,"name":"entries[" + item.no + "][user]","classes":"validate","fieldType":"text","validations":{"type":"text","max":{"value":65535}}};
    var userFullInput = {"id":"userWrapper_" + item.id,"classes":"input-field col s4 m4 l4","field":userInput,"label":userLabel};
...
function gmpPackingDocRegistryItemData(item){
    var dataLabel = {"type":"label","contents":{"type":"text","classes":"data_title"}};
    var dataInput = {"type":"textarea","id":"data_" + item.id,"name":"entries[" + item.no + "][data]","classes":"validate","fieldType":"text","validations":{"type":"text","max":{"value":65535}}};
...
function gmpPackingDocRegistryItemUrl(item){
    ...
    var urlInput = {"type":"textarea","id":"album_url_" + item.id,"name":"entries[" + item.no + "][url]","classes":"validate","fieldType":"text","validations":{"type":"text","max":{"value":65535}}};
...
function gmpPackingDocRegistryItemFile(item){
    var logoField = {"type":"file","id":"log_files_" + item.id,"classes":"select_logo_button","name":"entries[" + item.no + "][files]"};
