...
function loadLogForm(htmlElement){
    $server.request({
        service: 'log-gmp-packing-cold-room-temp',
...
function loadPrefilledLogForm(htmlElement, data){
    $server.request({
        service: 'authorization-report-gmp-packing-cold-room-temp',
        data: data,
        success: function(response) {
            if (response.meta.return_code == 0) {
                changesFlag = false;
...
function sendGmpPackingThermoCalibrationReport(){
    ...
    if(validateLog()){
        ...
        $server.request({
            service: 'capture-gmp-packing-cold-room-temp',
            data: report,
            success: function(response){
                if (response.meta.return_code == 0) {
...
function updateGmpPackingThermoCalibrationReport(reportID){
    ...
    if(validateLog()){
        ...
        $server.request({
            service: 'update-gmp-packing-cold-room-temp',
            data: report,
            success: function(response){
                if (response.meta.return_code == 0) {
                    Materialize.toast("Reporte enviado con exito", 3000, "rounded");
...
function gmpPackingThermoCalibrationReportHeader(){
    var header = {"type":"thead","rows":[{"type":"tr","columns":[{"type":"th","classes":"time_title timeColumn"},{"type":"th","classes":"number_title numberColumn"},{"type":"th","classes":"cold_room_test_title testColumn"},{"type":"th","classes":"deficiencies_title deficienciesColumn"},{"type":"th","classes":"action_title actionColumn"}]}]};
