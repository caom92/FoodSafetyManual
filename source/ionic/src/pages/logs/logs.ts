    ...
    this.storage.get("privileges").then(
      data => {
        ...
        if(this.selectedProgram.target == "log"){
          if (tempArray[item.title].suffix == "gmp-packing-preop" || tempArray[item.title].suffix == "gmp-packing-hand-washing" || tempArray[item.title].suffix == "gmp-packing-glass-brittle" || tempArray[item.title].suffix == "gmp-packing-scale-calibration" || tempArray[item.title].suffix == "gap-packing-preop" || tempArray[item.title].suffix == "gmp-packing-scissors-knives" || tempArray[item.title].suffix == "gmp-packing-thermo-calibration" || tempArray[item.title].suffix == "gmp-packing-cold-room-temp"  || tempArray[item.title].suffix == "gmp-self-inspection-pest-control" || tempArray[item.title].suffix == "gmp-others-unusual-occurrence" || tempArray[item.title].suffix == "gap-others-unusual-occurrence" || tempArray[item.title].suffix == "gmp-packing-aged-product" || tempArray[item.title].suffix == "gmp-packing-finished-product"  || tempArray[item.title].suffix == "gmp-doc-control-doc-control") {
