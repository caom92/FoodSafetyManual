...
export class GAPPackingPreopLogComponent implements OnInit {
  ...
  save() {
    if (this.gapPackingPreopForm.valid) {
      ...
      this.logService.send(this.gapPackingPreopForm.value, 'capture-gap-packing-preop', logDetails).then(success => {
      ...
      this.logService.setAsDirty(this.gapPackingPreopForm)
