...
export class GAPPackingPreopAreaInventoryComponent implements OnInit, OnDestroy {
  @Language()
  lang: string
  ...
  ngOnDestroy(){
    if (this.dragulaService.find("preopAreasBag") !== undefined){
      ...
      console.error("No Dragula bag present on gap-packing-preop Inventory")
    }
  }
  ...
  addItem(){
    let modal = this.modalController.create(GAPPackingPreopAddAreaComponent)
    modal.present()
