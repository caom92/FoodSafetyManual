...
export class GMPPackingThermoCalibrationAddItemComponent implements OnInit {
      ...
      let confirmAdd = this.alertCtrl.create({
        ...
        buttons: [
          ...
          },
          {
            ...
            handler: () => {
              loaderAdd.present()
              let data: {item:InventoryItem} = {item:{ id: 0, is_active: 1, name: this.newItem.value.name, position: 0 }}
