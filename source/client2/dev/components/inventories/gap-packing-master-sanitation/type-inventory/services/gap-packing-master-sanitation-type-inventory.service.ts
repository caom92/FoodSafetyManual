...
@Injectable()
export class GAPPackingMasterSanitationTypeInventoryService extends InventoryService {
  ...
  public getInventory(suffix: string): Promise<any> {
    let inventoryPromise = new Promise<any>((resolve, reject) => {
      this.apiService.service('inventory-type-' + suffix).then(success => {
        resolve(success)
      }, error => {
        reject(error)
      })
  ...
  public getInventoryByArea(suffix: string, data: any): Promise<any> {
    let inventoryPromise = new Promise<any>((resolve, reject) => {
      this.apiService.service('inventory-type-' + suffix, data).then(success => {
        resolve(success)
      }, error => {
        reject(error)
      })
  ...
  public toggleItem(suffix: string, data: any): Promise<any> {
    let togglePromise = new Promise<any>((resolve, reject) => {
      this.apiService.service('toggle-type-' + suffix, data).then(success => {
        if (data.is_active == 0) {
  ...
  public reorderInventory(suffix: string, data: any): Promise<any> {
    let reorderPromise = new Promise<any>((resolve, reject) => {
      this.apiService.service('reorder-type-' + suffix, { items: data }).then(success => {
        resolve(success)
      }, error => {
        if (error === Number(error)) {
          this.toastService.showClientMessage('reverse-bad-request', 1)
  ...
  public addItem(suffix: string, data: any): Promise<any> {
    let addPromise = new Promise<any>((resolve, reject) => {
      this.apiService.confirmationService('add-type-' + suffix, { key: 'Titles.add_item' }, { key: 'Messages.add_item', args: { item: data.name } }, data).then(success => {
        resolve(success)
      }, error => {
        reject(error)
      })
