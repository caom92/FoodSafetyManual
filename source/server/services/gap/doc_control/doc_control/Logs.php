...
namespace fsm\database\gap\docControl\docControl;
require_once realpath(dirname(__FILE__).'/../../../../dao/LogTable.php');
use fsm\database as db;


// Interfaz para la tabla gap_packing_doc_control_doc_control_logs
class Logs extends db\LogTable
{
  // Crea una instancia de una interfaz a la base de datos para modificar 
  // la tabla gap_packing_doc_control_doc_control_logs
  function __construct() { 
    parent::__construct('gap_doc_control_doc_control_logs');
  }
  ...
  function selectByCaptureDateID($dateID) {
    return parent::select(
      ...
      [
        '[><]gap_doc_control_doc_control_documents(d)' => [
          'document_id' => 'id'
        ]
      ]
    );
  ...
  function selectByDateIntervalLogIDAndZoneID($start, $end, $zoneID) {
    ...
    return parent::$dataBase->query("
      ...
      FROM
        $this->table
      INNER JOIN 
        gap_doc_control_doc_control_documents AS d
        ON document_id = d.id
      INNER JOIN
        captured_logs AS cl
        ON
